                     ...
                     order_val=NULL)
{
  ...
  if(length(abund)>1){
    ...
    if(N_taxa<length(unique_tax) ){
      abund[,paste("Unclassified",type)] <- rowSums(abund[(length(unique_tax)-(length(unique_tax)-N_taxa)+1):length(unique_tax)])
      abund <- abund[-(length(unique_tax)-(length(unique_tax)-N_taxa)+1):-length(unique_tax)]
    }
    if(no_other_type)  abund[,paste("Unclassified",type)] <- NULL
  }
  index <- !is.na(rownames(samp))
  ...
  if(p_val==TRUE & (bar_chart==FALSE | (bar_chart==TRUE & bar_chart_stacked==FALSE))){
    ...
    for (i in 1:length(unique(samp2$wrap))){
      ...
      if(stats=="non-parametric"){   #Kruskal-Wallis change to wilcoxon
  ...
  if(!is.null(color_by)){
    molten[molten$variable != paste("Unclassified",type),"colvar"] <- molten %>% dplyr::filter(variable != paste("Unclassified",type)) %>% .[,"variable"] %>% match(tax[,type]) %>% tax[.,color_by] %>% as.character
    molten[molten$variable == paste("Unclassified",type),"colvar"] <- paste("Unclassified",color_by) %>% as.character
  }
  ...
  if(bar_chart==TRUE){
    if(bar_chart_stacked==TRUE)
      p <-  ggplot(molten_mean,aes(x=factor(predictor2,labels=legend_names),y=value, fill=variable)) + 
        theme_bw()+geom_bar(stat="identity")+ theme_bw() + 
        theme(panel.grid.major = element_blank(),
              ...
              text=element_text(size=18)) +xlab(NULL)+ylab(ylabs)+ggtitle(main) +  scale_fill_manual(values =cols,labels=ordered) + guides(fill = guide_legend(title=NULL))
    if(bar_chart_stacked==FALSE){
      ...
      p <-  p+ theme_bw()  + theme(panel.grid.major = element_blank(),
                                   ...
                                   text=element_text(size=18)) +xlab(NULL)+ylab(ylabs)+ggtitle(main)+ theme(strip.background = element_blank()) +coord_flip()
    }
  }
  if(!is.null(facet_wrap))   {
    ...
    if(facet_n==TRUE){
      label_names <- samp2 %>%
        ...
        dplyr::summarise(n = n()) %>%
        dplyr::mutate(pasted_label = paste0("FT ", levels(factor(samp2[,facet_wrap])), ", n = ", n))
  ...
  if(log==TRUE){
    ...
    if(p_val==TRUE){
      if(p_adjust){
        if(log_max == 100)  p <- p+ scale_y_log10(breaks=c(.000001,.001,.01,.1,1,7,70),labels=c("0%","0.1%","1%","10%","100%", "P", "q-value"))
        if(log_max == 10)  p <- p+ scale_y_log10(breaks=c(.001,.01,.05,0.1,0.126,1.26),labels=c("0%","1%","5%","10%", "P", "q-value"))
        if(log_max == 1)  p <- p+ scale_y_log10(breaks=c(.001,.01,0.0126,0.126),labels=c("0%","1%", "P", "q-value"))
      }
      else{
        if(log_max == 100)  p <- p+ scale_y_log10(breaks=c(.000001,.001,.01,.1,1,7),labels=c("0%","0.1%","1%","10%","100%", "P"))
        if(log_max == 10)  p <- p+ scale_y_log10(breaks=c(.001,.01,.05,0.10,0.126),labels=c("0%","1%","5%","10%", "P"))
        if(log_max == 1)  p <- p+ scale_y_log10(breaks=c(.001,.01,0.0126),labels=c("0%","1%", "P"))
      }
    }
  }
  if(log==FALSE){
    ...
    if(p_val==TRUE){
      if(p_adjust==TRUE) p <- p + scale_y_continuous(breaks=c(0,.25,.50,.75,1,1.12,1.20),labels=c("0%","25%","50%","75%","100%", "P", "q-value"))
      if(p_adjust==FALSE) p <- p + scale_y_continuous(breaks=c(0,.25,.50,.75,1,1.12),labels=c("0%","25%","50%","75%","100%", "P"))
    }
  }
  ...
  if(p_stars==FALSE & p_val==TRUE & (bar_chart==FALSE | (bar_chart==TRUE & bar_chart_stacked==FALSE))){
    p <- p + geom_text(data=pval,aes(x=variable,y=y,label=ifelse(pval<0.05, paste(format.pval(pval,1,0.01,nsmall=2)),"")) ,size=3,hjust=1,fontface="bold")
    p <- p + geom_text(data=pval,aes(x=variable,y=y,label=ifelse(pval>=0.05, paste(format.pval(pval,1,0.01,nsmall=2)),"")) ,size=3,hjust=1)
    if(p_adjust){
      p <- p + geom_text(data=pval,aes(x=variable,y=y_adjust,label=ifelse(p_adjust<0.05, paste(format.pval(p_adjust,1,0.01,nsmall=2)),"")) ,size=3,hjust=1,fontface="bold")
      p <- p + geom_text(data=pval,aes(x=variable,y=y_adjust,label=ifelse(p_adjust>=0.05, paste(format.pval(p_adjust,1,0.01,nsmall=2)),"")) ,size=3,hjust=1)
    }
  }
  p
}

p_relative_for <- rabuplot(ps_rarefied,predictor = "fortification", main = NULL, colors = mycolors_for, facet_wrap = "FT", legend_title = "Fortification", legend_names = c("BC","CF"))
p_relative_BM <- rabuplot(ps_rarefied,predictor = "birth_mode", main = NULL, colors = mycolors_BM, facet_wrap = "FT", legend_title = "Birth mode", legend_names = c("CS","VB"))
