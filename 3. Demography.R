...
#baseline
################################### Table 1 fortifier
myvar_all <- c("GA", "BW", "SGA","PMA", "birth_mode", "multiplebirth_merge", "sex", "hospital", "probiotics_before")
myvar_FT <- c("antibiotics","MMpercent3d")
catvar <- c( "SGA", "birth_mode", "multiplebirth_merge", "sex", "hospital", "antibiotics", "probiotics_before", "fortification")
nonvar <- c("hospital")
...
myvar_all_BM <- c("GA", "BW", "SGA","PMA", "fortification", "multiplebirth_merge", "sex", "hospital", "probiotics_before")
tab_all_BM <- CreateTableOne(vars = myvar_all_BM, strata = "birth_mode", data = metadata_infant, factorVars = catvar)
sepline_all_BM <- print(tab_all_BM, showAllLevels = TRUE, exact = nonvar,quote = TRUE, noSpaces = TRUE, printToggle = FALSE)
#interventions
tab_FT0_BM <- CreateTableOne(vars = myvar_FT, strata = "birth_mode", data = metadata_FT0, factorVars = catvar)
sepline_FT0_BM <- print(tab_FT0_BM, showAllLevels = TRUE, exact = nonvar,quote = TRUE, noSpaces = TRUE, printToggle = FALSE)

tab_FT1_BM <- CreateTableOne(vars = myvar_FT, strata = "birth_mode", data = metadata_FT1, factorVars = catvar)
sepline_FT1_BM <- print(tab_FT1_BM, showAllLevels = TRUE, exact = nonvar,quote = TRUE, noSpaces = TRUE, printToggle = FALSE)

tab_FT2_BM <- CreateTableOne(vars = myvar_FT, strata = "birth_mode", data = metadata_FT2, factorVars = catvar)
...
probiotics_infant <- select(metadata_rarefied, record_id, hospital, probiotics_before)
probiotics_infant <- probiotics_infant[!duplicated(probiotics_infant$record_id),]
table(probiotics_infant$hospital,probiotics_infant$probiotics_before)
